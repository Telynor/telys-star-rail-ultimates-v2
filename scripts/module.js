...
function openInitiativePortraitEditor(actor){
  ...
  const dialog=new Dialog({title:`${actor.name} — Initiative Portrait`,content,buttons:{close:{label:"Close"}},render:html=>{
    ...
    const onWheel=event=>{if(!inStage(event))return;event.preventDefault();event.stopImmediatePropagation();const input=form.find('[name="carouselImageScale"]'),next=clamp((Number(input.val())||100)+(event.deltaY<0?5:-5),50,400);input.val(next);scheduleSave();};
