...
class BossHud {
  constructor(){this.element=null;this.drag=null;this.layerObserver=null;}
  placeBelowCarouselLayer(){
    if(!this.element)return;
    const dock=document.querySelector("#combat-dock");
    const uiTop=document.querySelector("#ui-top");
    if(game.modules.get("combat-tracker-dock")?.active && dock && uiTop){
      this.element.classList.add("tsru-boss-hud-in-carousel-layer");
      if(this.element.parentElement!==uiTop || this.element.nextElementSibling!==dock)uiTop.insertBefore(this.element,dock);
      return;
    }
    this.element.classList.remove("tsru-boss-hud-in-carousel-layer");
  ...
  destroy(){for(const timer of this.initialPlacementTimers??[])window.clearTimeout(timer);this.initialPlacementTimers=[];this.layerObserver?.disconnect();this.layerObserver=null;this.element?.remove();this.element=null;if(state.bossHud===this)state.bossHud=null;}
}
