...
function hubProfileImage(actor){if(!actor)return "icons/svg/mystery-man.svg";let token=null;try{token=actor.getActiveTokens?.(true,true)?.[0]??actor.getActiveTokens?.()?.[0];}catch{}return String(token?.document?.texture?.src||token?.texture?.src||actor.prototypeToken?.texture?.src||actor.img||"icons/svg/mystery-man.svg");}
...
const visibleQuest = quest => game.user.isGM || (!quest.hidden && (quest.actorIds ?? []).some(id => ownedCharacterIds().has(id)));
...
function refreshQuestWindows() {
  for (const app of Object.values(ui.windows ?? {})) if (["tsru-quest-log", "tsru-quest-manager", "tsru-quest-editor", "tsru-hub"].includes(app.options?.id)) app.render(false);
}
...
class HSRHub extends FormApplication {
  ...
  activateListeners(html) {
    ...
    html.find(".tsru-phone-scroll").on("wheel",event=>{
      ...
      event.currentTarget.scrollTop+=Number(event.originalEvent?.deltaY??event.deltaY??0);
    });
...
class HSRHubConfig extends FormApplication {
  ...
  activateListeners(html){
    ...
    html.find("[data-toggle-hub-double]").on("click",event=>{this.captureButtons(html);const index=Number(event.currentTarget.dataset.toggleHubDouble);this.buttonsDraft[index].double=!this.buttonsDraft[index].double;this.render(false);});
    ...
    const baseHeight=preview.innerHeight(),extraHeight=Math.max(360,Math.round(baseHeight*.65));canvas.height(baseHeight+extraHeight);canvas.find("[data-hub-button-index]").each((_index,node)=>{const index=Number(node.dataset.hubButtonIndex),x=Number(html.find(`[name="buttons.${index}.x"]`).val())||0,y=Number(html.find(`[name="buttons.${index}.y"]`).val())||0,width=Number(html.find(`[name="buttons.${index}.width"]`).val())||10,height=Number(html.find(`[name="buttons.${index}.height"]`).val())||16,double=html.find(`[name="buttons.${index}.double"]`).val()==="1";Object.assign(node.style,{left:`${x}%`,top:`${y/100*baseHeight}px`,width:`${Math.min(100,width*(double?2:1))}%`,height:`${height/100*baseHeight}px`});});
    ...
    html.find("[data-hub-button-size]").on("input change",event=>{const match=event.currentTarget.name.match(/^buttons\.(\d+)\.(width|height)$/);if(!match)return;const value=Math.max(match[2]==="width"?4:6,Math.min(100,Number(event.currentTarget.value)||10)),node=html.find(`[data-hub-button-index="${match[1]}"]`)[0],double=html.find(`[name="buttons.${match[1]}.double"]`).val()==="1";if(node)node.style[match[2]]=match[2]==="height"?`${value/100*baseHeight}px`:`${Math.min(100,value*(double?2:1))}%`;});
    let drag=null;
    ...
    preview.on("pointerdown","[data-hub-pose-preview]",event=>{if(event.button!==0)return;event.preventDefault();const rect=preview[0].getBoundingClientRect();drag={type:"pose",node:event.currentTarget,rect,startX:event.clientX,startY:event.clientY,x:numberOr(html.find('[name="poseX"]').val(),50),y:numberOr(html.find('[name="poseY"]').val(),50)};event.currentTarget.setPointerCapture?.(event.pointerId);});
    preview.on("pointerdown","[data-hub-wallpaper-preview]",event=>{if(event.button!==0||event.target.closest("[data-hub-button-index]"))return;event.preventDefault();const rect=event.currentTarget.getBoundingClientRect();drag={type:"wallpaper",node:event.currentTarget,rect,startX:event.clientX,startY:event.clientY,x:numberOr(html.find('[name="wallpaperX"]').val(),50),y:numberOr(html.find('[name="wallpaperY"]').val(),50)};event.currentTarget.setPointerCapture?.(event.pointerId);});
    preview.on("pointermove",event=>{if(!drag)return;if(drag.type==="button"){const maxY=(canvas.height()-drag.node.getBoundingClientRect().height)/baseHeight*100,x=snap((event.clientX-drag.canvasRect.left-drag.dx)/drag.canvasRect.width*100),y=snap((event.clientY-drag.canvasRect.top-drag.dy)/baseHeight*100),left=Math.max(0,Math.min(100,x)),top=Math.max(0,Math.min(maxY,y));drag.node.style.left=`${left}%`;drag.node.style.top=`${top/100*baseHeight}px`;html.find(`[name="buttons.${drag.index}.x"]`).val(left);html.find(`[name="buttons.${drag.index}.y"]`).val(top);}else if(drag.type==="resize"){const factor=html.find(`[name="buttons.${drag.index}.double"]`).val()==="1"?2:1,actualWidth=snap((drag.width+event.clientX-drag.startX)/drag.canvasRect.width*100),height=snap((drag.height+event.clientY-drag.startY)/baseHeight*100),safeActual=Math.max(4*factor,Math.min(100,actualWidth)),safeWidth=safeActual/factor,safeHeight=Math.max(6,Math.min(100,height));drag.node.style.width=`${safeActual}%`;drag.node.style.height=`${safeHeight/100*baseHeight}px`;html.find(`[name="buttons.${drag.index}.width"]`).val(safeWidth);html.find(`[name="buttons.${drag.index}.height"]`).val(safeHeight);}else if(drag.type==="wallpaper"){const x=Math.max(0,Math.min(100,drag.x+(event.clientX-drag.startX)/drag.rect.width*100)),y=Math.max(0,Math.min(100,drag.y+(event.clientY-drag.startY)/drag.rect.height*100));html.find('[name="wallpaperX"]').val(Math.round(x));html.find('[name="wallpaperY"]').val(Math.round(y));preview.css({"--hub-wallpaper-x":`${x}%`,"--hub-wallpaper-y":`${y}%`});}else{const x=Math.max(-100,Math.min(200,drag.x+(event.clientX-drag.startX)/drag.rect.width*100)),y=Math.max(-100,Math.min(200,drag.y+(event.clientY-drag.startY)/drag.rect.height*100));html.find('[name="poseX"]').val(Math.round(x));html.find('[name="poseY"]').val(Math.round(y));drag.node.style.setProperty("--pose-x",`${x}%`);drag.node.style.setProperty("--pose-y",`${y}%`);}}).on("pointerup pointercancel",()=>{drag=null;});
    ...
    preview.on("wheel","[data-hub-wallpaper-preview]",event=>{event.preventDefault();event.stopPropagation();const input=html.find('[name="wallpaperScale"]'),value=Math.max(25,Math.min(400,numberOr(input.val(),100)+(event.originalEvent.deltaY<0?5:-5)));input.val(value);preview.css("--hub-wallpaper-scale",value);});
    preview.on("dragover",event=>{event.preventDefault();$(event.target).closest("[data-hub-wallpaper-preview]").addClass("is-dragover");}).on("dragleave",event=>$(event.target).closest("[data-hub-wallpaper-preview]").removeClass("is-dragover")).on("drop",event=>{event.preventDefault();const phone=$(event.target).closest("[data-hub-wallpaper-preview]");phone.removeClass("is-dragover");let data={};try{data=JSON.parse(event.originalEvent.dataTransfer.getData("text/plain")||"{}");}catch{}const path=data.path||data.src||data.texture?.src;if(!path)return;if(phone.length){html.find('[name="wallpaper"]').val(path);updateWallpaper();}else{html.find('[name="poseImage"]').val(path).trigger("change");preview.find("[data-hub-pose-preview] img").attr("src",path);}});
    html.find('[name="poseImage"]').on("change",event=>preview.find("[data-hub-pose-preview] img").attr("src",event.currentTarget.value));
  }
  async _updateObject(_event,formData){if(!game.user.isGM)return ui.notifications.error("Only a GM can save player phone layouts.");const expanded=foundry.utils.expandObject(formData),stored=clone(game.settings.get(MODULE_ID,"hsrHubConfig")??{});stored.snap=Boolean(expanded.snap);stored.gridSize=Math.max(1,Math.min(25,numberOr(expanded.gridSize,4)));stored.poseByActor??={};stored.poseByActor[this.actorId]={image:String(expanded.poseImage||""),x:numberOr(expanded.poseX,50),y:numberOr(expanded.poseY,50),scale:Math.max(20,Math.min(500,numberOr(expanded.poseScale,100))),flip:Boolean(expanded.poseFlip)};stored.phoneByActor??={};stored.phoneByActor[this.actorId]={wallpaper:String(expanded.wallpaper||""),wallpaperFit:String(expanded.wallpaperFit||"cover"),wallpaperX:numberOr(expanded.wallpaperX,50),wallpaperY:numberOr(expanded.wallpaperY,50),wallpaperScale:Math.max(25,Math.min(400,numberOr(expanded.wallpaperScale,100))),buttons:Object.values(expanded.buttons??{}).map(button=>({id:String(button.id||foundry.utils.randomID()),label:String(button.label||"Button"),icon:String(button.icon||"fas fa-star"),action:String(button.action||"sheet"),target:String(button.target||""),double:String(button.double||"0")==="1",x:numberOr(button.x,0),y:numberOr(button.y,0),width:Math.max(4,numberOr(button.width,10)),height:Math.max(6,numberOr(button.height,16))}))};await game.settings.set(MODULE_ID,"hsrHubConfig",stored);refreshQuestWindows();const actor=game.actors.get(this.actorId),name=this.actorId==="__gm__"?"DM PANEL":actor?.name||"Character";ui.notifications.info(`${name}'s HSR Phone Hub layout saved.`);}
}
...
class HSRMessenger extends Application {
  ...
  async openContact(actorId){
    const contactIds=messengerContactIds();
    if(!contactIds.has(actorId))return ui.notifications.warn("That contact is not currently available.");
    if(game.user.isGM){const thread=messengerThreads().find(entry=>entry.contactActorId===actorId||(entry.memberActorIds??[]).includes(actorId));if(!thread)return ui.notifications.warn("No conversation with that contact exists yet. Use Make Group Chat to start one as the DM.");this.threadId=thread.id;this.view="messages";this.render(false);return;}
    const mainActor=api()?.getSelectedMainCharacter?.(),contact=game.actors.get(actorId);
    if(!mainActor||!contact)return ui.notifications.warn("Select a main character before opening a conversation.");
    const thread=messengerThreads().find(entry=>entry.contactActorId===actorId&&(entry.memberActorIds??[]).includes(mainActor.id)&&(entry.participantIds??[]).includes(game.user.id));
    if(thread){this.threadId=thread.id;this.view="messages";this.render(false);return;}
    if(!partyAuthority())return ui.notifications.error("An active GM is required to start a new conversation.");
    game.socket.emit(SOCKET,{type:"messengerOpenContact",userId:game.user.id,mainActorId:mainActor.id,contactActorId:contact.id,requestedThreadId:foundry.utils.randomID()});
  ...
  async sendMessage(html){
    ...
    const message={id:foundry.utils.randomID(),senderUserId:game.user.id,speakerActorId:actor.id,speakerName:actor.name,text,createdAt:Date.now()};
    if(!game.user.isGM){if(!partyAuthority())return ui.notifications.error("An active GM is required to send messages.");game.socket.emit(SOCKET,{type:"messengerSendMessage",userId:game.user.id,threadId:thread.id,actorId:actor.id,message});html.find('[name="messageText"]').val("");return;}
    thread.messages??=[];thread.messages.push(message);thread.updatedAt=Date.now();await saveMessengerThreads(threads);this.render(false);
  }
...
class QuestEditor extends FormApplication {
  constructor(questId="") { super(); this.selectedQuestId = questId; }
  static get defaultOptions() { return foundry.utils.mergeObject(super.defaultOptions, {id: "tsru-quest-editor", title: "Edit Mission", template: `modules/${MODULE_ID}/templates/quest-editor.hbs`, width: 780, height: 760, resizable: true, closeOnSubmit: false, dragDrop: [{dropSelector: ".tsru-reward-drop"}]}); }
  async getData() {
    ...
    return {quests: all.map(q => ({...q, searchText: [q.title,q.location,q.description,...q.rewards.map(r=>r.name)].join(" ").toLocaleLowerCase()})),
      selectedQuest: selected ? {...selected, archived:selected.status==="complete"&&selected.rewardsClaimed,typeOptions:typeList.map(t=>({...t,selected:t.id===selected.typeId})), rarityOptions:rarityList, mainPartyActors:mainParty.map(actorOption), otherActors:otherActors.map(actorOption), currentStage:selected.stages[selected.stageIndex]??selected.stages[0]} : null};
  }
  activateListeners(html) {
    super.activateListeners(html); activatePickers(html);
    html.find("[data-toggle-quest-hidden]").on("click",async()=>{const all=quests(),q=all.find(entry=>entry.id===this.selectedQuestId);if(!q)return;q.hidden=!q.hidden;await this.persist(all,false,q);this.render(false);});
...
class QuestManager extends FormApplication {
  constructor(...args){super(...args);this.search="";this.filter="all";}
  static get defaultOptions(){return foundry.utils.mergeObject(super.defaultOptions,{id:"tsru-quest-manager",title:"Mission Manager — GM",template:`modules/${MODULE_ID}/templates/quest-manager.hbs`,width:920,height:730,resizable:true});}
  getData(){
    const all=quests().map(normalizeQuest),typeList=types();
    for(const quest of all)if(!typeList.some(type=>type.id===quest.typeId))typeList.push({id:quest.typeId,name:"Other Mission",icon:"icons/svg/book.svg",background:""});
    const groups=typeList.map(type=>({type,missions:all.filter(quest=>quest.typeId===type.id).map(quest=>({...quest,typeName:type.name,typeIcon:type.icon,cardBackground:quest.background||type.background||"",archived:quest.status==="complete"&&quest.rewardsClaimed,managerStatus:quest.status==="complete"&&quest.rewardsClaimed?"archived":quest.hidden?"hidden":quest.status==="complete"?"complete":"active",searchText:`${quest.title} ${quest.location} ${type.name} ${quest.status}`.toLowerCase()}))})).filter(group=>group.missions.length);
    return {groups,total:all.length,hiddenCount:all.filter(quest=>quest.hidden).length,archivedCount:all.filter(quest=>quest.status==="complete"&&quest.rewardsClaimed).length};
  }
  activateListeners(html){super.activateListeners(html);html.find("[data-new-quest]").on("click",async()=>{const all=quests(),q=normalizeQuest({hidden:true,actorIds:questActors().filter(actor=>Boolean(actorUltimate(actor).mainParty)).map(actor=>actor.id)});all.push(q);await saveQuests(all,{notify:false});new QuestEditor(q.id).render(true);});html.find("[data-open-manager-quest]").on("click",event=>new QuestEditor(event.currentTarget.dataset.openManagerQuest).render(true));html.find("[data-mission-search]").val(this.search);html.find("[data-manager-filter]").val(this.filter);const applyFilter=()=>{const query=this.search.toLowerCase().trim();html.find("[data-manager-card]").each((_index,card)=>{card.hidden=Boolean((query&&!card.dataset.managerSearchText.includes(query))||(this.filter!=="all"&&this.filter!==card.dataset.managerStatus));});html.find("[data-manager-group]").each((_index,group)=>group.hidden=![...group.querySelectorAll("[data-manager-card]")].some(card=>!card.hidden));};html.find("[data-mission-search]").on("input",event=>{this.search=event.currentTarget.value;applyFilter();});html.find("[data-manager-filter]").on("change",event=>{this.filter=event.currentTarget.value;applyFilter();});applyFilter();}
  async _updateObject(){}
}
...
class QuestLog extends FormApplication {
  ...
  async getData() {
    ...
    if(this.filter==="archived")list=list.filter(q=>q.status==="complete"&&q.rewardsClaimed);
    else {list=list.filter(q=>!(q.status==="complete"&&q.rewardsClaimed));if(this.filter!=="all")list=list.filter(q=>q.typeId===this.filter);}
    ...
    return {allIcon:game.settings.get(MODULE_ID,"questAllIcon"),types:typeList.map(t=>({...t,active:this.filter===t.id})),allActive:this.filter==="all",archivedActive:this.filter==="archived",archivedMode:this.filter==="archived",groups,flat:enriched,selected:selected?enrich(selected):null,allMode:this.filter==="all",gm:game.user.isGM};
  }
  activateListeners(html) {
    super.activateListeners(html);
...
Hooks.once("ready",()=>{
  game.socket.on(SOCKET,async payload=>{
    ...
    if(payload?.type==="messengerOpenContact" && game.user.isGM && partyAuthority()?.id===game.user.id){
      ...
      const threads=messengerThreads();let thread=threads.find(entry=>entry.contactActorId===contact.id&&(entry.memberActorIds??[]).includes(mainActor.id)&&(entry.participantIds??[]).includes(user.id));
      if(!thread){const now=Date.now();thread={id:String(payload.requestedThreadId||foundry.utils.randomID()),title:contact.name,contactActorId:contact.id,memberActorIds:[mainActor.id,contact.id],participantIds:[user.id],createdBy:user.id,createdAt:now,updatedAt:now,messages:[]};threads.push(thread);await saveMessengerThreads(threads);}
      game.socket.emit(SOCKET,{type:"messengerThreadOpened",userId:user.id,threadId:thread.id});return;
    }
    if(payload?.type==="messengerThreadOpened"){if(String(payload.userId)!==game.user.id)return;for(const app of Object.values(ui.windows??{}))if(app instanceof HSRMessenger){app.threadId=String(payload.threadId||"");app.view="messages";app.render(false);}return;}
    if(payload?.type==="messengerSendMessage" && game.user.isGM && partyAuthority()?.id===game.user.id){
      const user=game.users.get(String(payload.userId||"")),actor=game.actors.get(String(payload.actorId||"")),threads=messengerThreads(),thread=threads.find(entry=>entry.id===String(payload.threadId||""));
      if(!user||user.isGM||!actor||!thread||!actor.testUserPermission(user,"OWNER")||!(thread.participantIds??[]).includes(user.id))return;
      const text=String(payload.message?.text||"").trim().slice(0,4000);if(!text)return;
      thread.messages??=[];thread.messages.push({id:String(payload.message?.id||foundry.utils.randomID()),senderUserId:user.id,speakerActorId:actor.id,speakerName:actor.name,text,createdAt:Number(payload.message?.createdAt)||Date.now()});thread.updatedAt=Date.now();await saveMessengerThreads(threads);return;
    }
