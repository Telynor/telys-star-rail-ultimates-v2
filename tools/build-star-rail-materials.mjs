...
const all=[...docs.values()].sort((a,b)=>a.name.localeCompare(b.name)),db=new ClassicLevel(packPath,{valueEncoding:"json"});await db.open();for(const x of all){await db.put(x._id,x);await writeFile(`${sourcePath}/${x._id}.json`,JSON.stringify(x,null,2)+"\n");}await db.close();await writeFile(`${root}/data/star-rail-recipes.json`,JSON.stringify(recipes,null,2)+"\n");
